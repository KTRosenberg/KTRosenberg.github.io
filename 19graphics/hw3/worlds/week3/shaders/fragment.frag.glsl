...
#define should_refract(mat_) mat_.refraction.rgb != vec3(0.0)
...
{
    return Ray(normalize(origin), normalize(direction));
}
    ...
    spheres[1] = Sphere(
        ...
        Material(
           ...
           6.0,
            ...
            vec4(1.0, 1.0, 1.0, 1.0),
            vec4(vec3(0.5), IDX_REFRACT_SAPPHIRE)
        )
    );
