...
async function setup(state) {
    ...
    hotReloadFile(getPath("KTRosenberg_hw8.js"));
    ...
    const bgMusic = new Audio(getPath('resources/audio/cor.mp3'));
    bgMusic.loop = true;
    ...
    state.fog_color_factor = 1;
    state.fog_color = [state.fog_color_factor * 53.0 / 255.0, state.fog_color_factor * 81.0 / 255.0, state.fog_color_factor * 192.0 / 255.0, 1.0];

    // load initial images, then continue setup after waiting is done
    const images = await imgutil.loadImagesPromise([
        getPath("resources/textures/toon_water.png"),
        getPath("resources/textures/card.jpg"),
        getPath("resources/textures/FRLAB.png")
    ]);
    ...
    state.world_dimensions = new Float32Array([256.0, 256.0]);
    ...
    if (!libSources) {
     ...
     //                       mode : 
                    ...
                    state.u_world_dimensionsLoc = gl.getUniformLocation(program, 'u_world_dimensions');
                    gl.uniform2fv(state.u_world_dimensionsLoc, state.world_dimensions);

                    state.uViewPositionLoc = gl.getUniformLocation(program, 'uViewPosition');
                    gl.uniform3fv(state.uViewPositionLoc, [0, 0, 0]);
                    state.u_ambientLoc = gl.getUniformLocation(program, 'u_ambient');
                } 
            },      
            {
    ...
    function addCubeToBufferLayout(state, lb, layouts, fnName, count = 1) {
        ...
        lb.indexByteOffset  += entry.indexCount * Uint32Array.BYTES_PER_ELEMENT * count;
    ...
    function generateParametricPlane(vertices, offset, u, v, opts = {}) {
        ...
        vertices[offset]     = ((u * 2) - 1) + xOffset;
        vertices[offset + 1] = yOffset;
        vertices[offset + 2] = -((v * 2) - 1) + -zOffset;

        vertices[offset + 3] = 0;
        vertices[offset + 4] = 1;
        vertices[offset + 5] = 0;

        vertices[offset + 6] = u + uOffset;
        vertices[offset + 7] = v + vOffset;
    }
    ...
    function generateParametricGeometryIndexedTriangleStrip(idxOffset, rows, cols, fnEvaluate, opts) {
        ...
        const indices = []//new Uint32Array((triCount * 3) + ((rows - 2) * 3));
...

        {
            ...
            info.indices  = new Uint32Array(indices);
    ...
    const ParametricTriangleStripType = {
        ...
        Plane : generateParametricPlane || function() {}
    }
    ...
    addParametricGeometryToBufferLayout(
        state, state.layoutBuilder, state.layouts, planeInfo, "drawPlane"
    );
...
function updateFallingCubeWithBasicGravity(state) {
    ...
    state.world.objInfo.position[1] = Math.max(-0.5, state.world.objInfo.position[1]);
}
...
function onStartFrame(t, state) {
    ...
    gl.uniform4fv(state.fog_colorLoc, state.fog_color);
    gl.uniform3fv(state.uViewPositionLoc, new Float32Array(pos));
...
function onDraw(t, projMat, viewMat, state, eyeIdx) {
    ...
    state.world_dimensions[0] = 256.0;
    state.world_dimensions[1] = 256.0;
    gl.uniform2fv(state.u_world_dimensionsLoc, state.world_dimensions);
    M.save(); 
    {
        // floor (We'd like a floor for the virtual world)
        {
            M.save();
                ...
                Mat.scale(M.matrix(), state.world_dimensions[0], 1.0, state.world_dimensions[1]);
        ...
        M.save();
                Mat.translate(M.matrix(), -1 + 1.1 * cos(state.time), -1.2 + 0.2 * sin(state.time), -5 + cos(state.time + 3.0));
                Mat.rotateX(M.matrix(), Math.PI / 2);
                Mat.rotateY(M.matrix(), sin(state.time) * 0.2);
        ...
        {   
            ...
            // but most of the time you would use UNSIGNED_INT or UNSIGNED_INT
            if (state.world.objInfo.isSelected || true) {
            ...
            M.save();
                ...
                Mat.translate(M.matrix(), -1 + 1.1 * cos(state.time), 0.0, -5 + cos(state.time + 3.0));


                Mat.translate(M.matrix(), 0.0, state.world.objInfo.position[1], 0.0);
                Mat.rotateX(M.matrix(), -Math.PI / 2);
                Mat.rotateY(M.matrix(), -sin(state.time) * 0.2);
                Mat.scale(M.matrix(), 0.9, 0.9, 0.9)
                gl.uniformMatrix4fv(state.uModelLoc, false, 
                    M.matrix()
                );
                state.drawCube();
